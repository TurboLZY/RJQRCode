...
@interface RJScannerViewController ()<UIImagePickerControllerDelegate, UINavigationControllerDelegate>
/// 名片字符串
...
@implementation RJScannerViewController {
    /// 扫描框
    RJScannerBorder *scannerBorder;
    /// 扫描器
    RJScanner *scanner;
    /// 提示标签
    UILabel *tipLabel;
...
- (void)viewDidLoad {
    ...
    scanner = [RJScanner scanerWithView:self.view scanFrame:scannerBorder.frame completion:^(NSString *stringValue) {
        // 完成回调
...
- (void)prepareScanerBorder {
    ...
    scannerBorder = [[RJScannerBorder alloc] initWithFrame:CGRectMake(0, 0, width, width)];
    scannerBorder.center = self.view.center;
    
    [self.view addSubview:scannerBorder];
    
    RJScannerMaskView *maskView = [RJScannerMaskView maskViewWithFrame:self.view.bounds cropRect:scannerBorder.frame];
